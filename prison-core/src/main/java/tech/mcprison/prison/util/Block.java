...
import tech.mcprison.prison.block.Lever;
...
public enum Block {
    ...
    Block(int legacyId, String id, int data) {
        this.legacyId = legacyId;
        this.id = id;
    ...
    /**
     * Retrieves the block state for a block at a specific location.
     *
     * @param loc The location of the block.
     * @return The {@link BlockState}, or null if the block doesn't have one.
     */
    public BlockState getBlockState(Location loc) {
        Block block = loc.getWorld().getBlockAt(loc);
        switch (block) {
            case LEVER:
                return new Lever(loc);
            default:
                return null;
        }
    }
