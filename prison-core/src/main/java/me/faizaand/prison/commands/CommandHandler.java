...
package me.faizaand.prison.commands;

import me.faizaand.prison.Prison;
import me.faizaand.prison.commands.handlers.*;
...
public class CommandHandler {
    ...
    public CommandHandler() {
        ...
        registerArgumentHandler(GamePlayer.class, new PlayerArgumentHandler());
        registerArgumentHandler(GameWorld.class, new WorldArgumentHandler());
    ...
    public void registerCommands(Object commands) {
        for (Method method : commands.getClass().getDeclaredMethods()) {
            ...
            Optional<PluginCommand> rootPcommandOptional =
                plugin.getPlatform().getCommandManager().getCommand(identifiers[0]);
            ...
            if (!rootPcommandOptional.isPresent()) {
                rootPcommand = new PluginCommand(identifiers[0], commandAnno.description(),
                    "/" + identifiers[0]);
                plugin.getPlatform().getCommandManager().registerCommand(rootPcommand);
            } else {
    ...
    public boolean onCommand(CommandSender sender, PluginCommand command, String label,
        ...
        if (rootCommand.onlyPlayers() && !(sender instanceof GamePlayer)) {
