...
public class WorldArgumentHandler extends ArgumentHandler<GameWorld> {
    ...
    public WorldArgumentHandler() {
        addVariable("sender", "The command executor", (sender, argument, varName) -> {
            ...
            return ((GamePlayer) sender).getLocation().getWorld();
        });
    }

    @Override
    public GameWorld transform(CommandSender sender, CommandArgument argument, String value)
            throws TransformError {
        return Prison.get().getPlatform().getWorldManager().getWorld(value).orElseThrow(() -> new TransformError(
