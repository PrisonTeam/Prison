...
public class ResetTimer {
    ...
    public void cancelAll() {
        ...
        for (int id : individualTaskIds.values()) Prison.getInstance().getPlatform().getScheduler().cancelTask(id);
    }
    ...
    public void registerIndividualMine(Mine mine) {
        int taskId = Prison.getInstance().getPlatform().getScheduler().runTaskTimerAsync(() -> this.reset(mine),
    ...
    public void unregisterIndividualMine(Mine mine) {
        Prison.getInstance().getPlatform().getScheduler().cancelTask(individualTaskIds.get(mine.getName()));
    ...
    private void initSyncTask() {
        this.syncTaskId = Prison.getInstance().getPlatform().getScheduler().runTaskTimerAsync(
                ...
                minesModule.getConfig().resetTimeSeconds,
                minesModule.getConfig().resetTimeSeconds
        );
    }
