...
public class CellsModule extends Module {
    ...
    public CellUser getUser(UUID uuid) {
        return users.stream().filter(cellUser -> cellUser.getUUID().getMostSignificantBits() == uuid
            .getMostSignificantBits()).findFirst().orElse(null);
    }
    ...
    public void saveUser(CellUser user) {
        ...
        try {
            ...
            Prison.getInstance().getPlatform()
                .log("&cThe user file %s could not be saved. Here's the stack trace:",
                    file.getName());
            e.printStackTrace();
        }
    }
    ...
    public Cell getCellByDoorLocation(Location loc) {
        return cells.stream().filter(cell -> cell.getDoorLocation().equals(loc)).findFirst()
            .orElse(null);
    }
    ...
    public void saveCell(Cell cell) {
        ...
        try {
            ...
            Prison.getInstance().getPlatform()
                .log("&cThe cell file %s could not be saved. Here's the stack trace:",
                    file.getName());
            e.printStackTrace();
        }
    }
    ...
    private void loadUsers() {
        ...
        for (File file : userFiles) {
            try {
                ...
                Prison.getInstance().getPlatform()
                    .log("&cThe user file %s could not be loaded. Here's the stack trace:",
                        file.getName());
                e.printStackTrace();
            }
        }
    ...
    private void loadCells() {
        ...
        for (File file : cellFiles) {
            try {
                ...
                Prison.getInstance().getPlatform()
                    .log("&cThe cell file %s could not be loaded. Here's the stack trace:",
                        file.getName());
                e.printStackTrace();
            }
        }
