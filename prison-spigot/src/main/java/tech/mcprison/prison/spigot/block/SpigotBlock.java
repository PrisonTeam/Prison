	...
	extends PrisonBlock
{
	...
	public static SpigotBlock getSpigotBlock(org.bukkit.block.Block bukkitBlock) {
		...
		if (bukkitBlock != null) {
			...
			if (xMat == null) {
				for (CustomBlockIntegration custIntegration : Prison.get().getIntegrationManager()
						.getCustomBlockIntegrations()) {
	...
	public String toString() {
		...
		sb.append(getPrisonBlock().getBlockNameSearch()).append(" ").append(getLocation().toWorldCoordinates());

		if (getChance() > 0) {
			sb.append(" chance:").append(Double.toString(getChance()));
		}

		if (getSalePrice() > 0) {
			sb.append(" sell:").append(Double.toString(getSalePrice()));
		}

		if (getPurchasePrice() > 0) {
			sb.append(" purch:").append(Double.toString(getPurchasePrice()));
		}
	...
	private PrisonBlock getPrisonBlockFromCustomBlockIntegration(PrisonBlockType blockType) {
		...
		switch (blockType) {
		case minecraft:
			// No special processing for minecraft types since that will be the fallback
			// later on:

		case CustomItems:
		case ItemsAdder: {
			CustomBlockIntegration customItemsIntegration = PrisonAPI.getIntegrationManager()
					.getCustomBlockIntegration(blockType);
			// NOTE: This would be the situation where the admin added the Custom Items
			// plugin, added blocks
...
//			break;
	...
	public void setPrisonBlock(PrisonBlock prisonBlock) {
		...
		case ItemsAdder: {
			CustomBlockIntegration customItemsIntegration = PrisonAPI.getIntegrationManager()
					.getCustomBlockIntegration(prisonBlock.getBlockType());
			...
			if (results != null) {
				...
				Output.get().logInfo("SpigotBLock.setPrisonBlock: Failed to set a custom block %s ",
						prisonBlock.getBlockNameFormal());
			}
		}
	...
	/**
	 * <p>
	 * This clears the drops for the given block, so if the event is not canceled,
	 * it will not result in duplicate drops.
	 * </p>
	 */
