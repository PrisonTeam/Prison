...
public class PrisonDebugBlockInspector
{
	...
	public void debugBlockBreak(SpigotPlayer player, boolean isSneaking, Location location) {
		...
		if (mine == null) {
			...
			output.add(String.format("-&dDebugBlockInfo: &7Not in a mine. &5%s &7%s", sBlock.getBlockName(),
...
//        	checkForCustomBlock( sBlock, targetBlock );
			...
			String m1 = String.format("-&dDebugBlockInfo:  &3Mine &7%s  &3Rank: &7%s  " + "&5%s  &7%s ", mine.getName(),
					(mine.getRank() == null ? "---" : mine.getRank().getName()), sBlock.getBlockName(),
					location.toWorldCoordinates());
...
//			MineTargetPrisonBlock tBlock = mine.getTargetPrisonBlock( sBlock );
			...
			if (targetBlock == null) {
				...
				String message = String.format(
						"-    &3TargetBlock: &7%s  " + "&3Mined: %s%b  &3Broke: &7%b  &3Counted: &7%b",
						targetBlock.getPrisonBlock().getBlockName(), (targetBlock.isMined() ? "&d" : "&2"),
						targetBlock.isMined(), targetBlock.isAirBroke(), targetBlock.isCounted());

				output.add(message);

				String message2 = String.format("-    &3isEdge: &7%b  " + "&3Exploded: %s%b  &3IgnoreAllEvents: &7%b",

						targetBlock.isEdge(), (targetBlock.isExploded() ? "&d" : "&2"), targetBlock.isExploded(),
		...
		if (!isSneaking) {
			output.add(String.format("  &d(&7Sneak to test BlockBreakEvent with block.&d)"));
		}
	...
	public void dumpBlockBreakEvent(SpigotPlayer player, SpigotBlock sBlock, MineTargetPrisonBlock targetBlock,
		...
		org.bukkit.inventory.ItemStack heldItem = SpigotCompatibility.getInstance()
				.getItemInMainHand(player.getWrapper());
		...
		boolean useShovel = blockName.matches("^clay$|farmland|grass_block|dirt|gravel|mycelium|"
				+ "podzol|^sand$|^red_sand$|soul_sand|soul_soil|" + "concrete_powder|^snow$|snow_block|powder_snow");

//    	XMaterial.

		boolean useAxe = blockName.matches("wood$|acacia|birch|jungle|spruce|leaves|crimson|"
				+ "sapling|bamboo|ladder|vine|bed$|fence|chest$|" + "table|bookshelf|jack_o_lantern|^melon$|^pumpkn$|"
				+ "sign|^cocoa$|mushroom_block|note_block|campfire|" + "banner|beehive|loom|barrel|jukebox|composter|"
				+ "daylight_detector");
		...
		SpigotItemStack tool = !isMineWand ? (SpigotItemStack) ourItem
				: useShovel ? new SpigotItemStack(XMaterial.DIAMOND_SHOVEL.parseItem())
						: (useAxe ? new SpigotItemStack(XMaterial.DIAMOND_AXE.parseItem())
		...
		output.add(String.format("&dBlockBreakEvent Dump: &7%s &3%s", checkBlock.getBlockName(),
				checkBlock.getLocation().toBlockCoordinates()));

		output.add(String.format("  &3Tool Used for drops: &2%s", tool.getName()));

		EventDropsStatus isNs = isDropCanceled(bbe);

		output.add("   &3Legend: &7EP&3: EventPriority  &7EC&3: EventCanceled  "
				+ "&7DC&3: DropsCanceled  &7EB&3: EventBlock  &7Ds&3: Drops  " + "&7ms&3: dur ms"
		...
		for (RegisteredListener listener : bbe.getHandlers().getRegisteredListeners()) {
			...
			try {
				...
				output.add(String.format("  &cError calling event: &3%s  &2[%s]", listener.getPlugin().getName(),
						e.getMessage()));
			...
			printEventStatus(bbe, listener.getPlugin().getName(), listener.getPriority().name(), checkBlock,
					targetBlock, tool, output, player, durationNano);
	...
	private void printEventStatus(BlockBreakEvent bbe, String plugin, String priority, SpigotBlock sBlock,
			MineTargetPrisonBlock targetBlock, SpigotItemStack tool, List<String> output, SpigotPlayer player,
			double durationNano) {
		...
		String eventBlockName = eventBlock == null ? "&4none" : eventBlock.getBlockNameFormal();
...
//    		List<ItemStack> drops = sBlk.getDrops( tool );
		...
		String msg = String.format(
				" &3Plugin: &7%-15s &2EP: &7%-9s  " + "&2EC: &7%5s  &2DC: &7%s  &aEB: &b%s  &aDs: %s  &ams: &7%s",
				plugin, (priority == null ? "$dnone" : priority), (isCanceled ? "&4true " : "false"), dropStats,
				eventBlockName, sb2, durationNanoStr);
		sb.append(msg);
