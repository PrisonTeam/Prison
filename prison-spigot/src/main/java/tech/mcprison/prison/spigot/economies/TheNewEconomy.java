...
public class TheNewEconomy extends EconomyCurrencyIntegration {
  ...
  public void integrate() {
    if(isRegistered()) {
      ...
      final boolean econAlreadySet = PrisonAPI.getIntegrationManager().getForType(IntegrationType.ECONOMY).isPresent();

      if(!econAlreadySet) {
        this.wrapper = new TheNewEconomyWrapper();
      } else {
        Output.get().logInfo("TheNewEconomy is not directly enabled - Available as backup. ");
