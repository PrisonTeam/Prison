...
public class SpigotBlockGetAtLocation {
	...
	/**
	 * <p>
	 * This should be the ONLY usage in the whole Prison plugin that gets the bukkit
	 * block from the world and converts it to a SpigotBlock..
	 * </p>
	 * 
	 * <p>
	 * This gets the actual block from the world, but it only reads, and does not
	 * update. I cannot say this is safe to run asynchronously, but so far I have
	 * not see any related problems when it is.
	 * 
	 */
	public Block getBlockAt(Location location, boolean containsCustomBlocks, SpigotWorld world) {
		...
		if (location != null) {
			...
			if (containsCustomBlocks) {
				...
				List<CustomBlockIntegration> cbIntegrations = PrisonAPI.getIntegrationManager()
