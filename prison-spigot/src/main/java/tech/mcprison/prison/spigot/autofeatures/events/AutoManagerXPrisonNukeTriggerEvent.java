...
public class AutoManagerXPrisonNukeTriggerEvent extends AutoManagerFeatures implements PrisonEventManager {
...
//	private NukeTriggerEvent nte;
	...
	public class AutoManagerXPrisonNukeTriggerEventListener extends AutoManagerXPrisonNukeTriggerEvent
			implements Listener {
		...
		public void onXPrisonNukeTriggerEvent(NukeTriggerEvent e, BlockBreakPriority bbPriority) {

			if (isDisabled(e.getPlayer().getLocation().getWorld().getName()) || bbPriority.isDisabled()) {
				return;
			}
...
//	boolean isEventEnabled = eP != null && !"DISABLED".equalsIgnoreCase( eP );
	...
	private void createListener(BlockBreakPriority bbPriority) {
		...
		AutoManagerXPrisonNukeTriggerEventListener autoManagerListener = new AutoManagerXPrisonNukeTriggerEventListener(
				bbPriority);

		pm.registerEvent(NukeTriggerEvent.class, autoManagerListener, ePriority, new EventExecutor() {
			public void execute(Listener l, Event e) {

				NukeTriggerEvent exEvent = (NukeTriggerEvent) e;

				((AutoManagerXPrisonNukeTriggerEventListener) l).onXPrisonNukeTriggerEvent(exEvent, bbPriority);
			}
		}, prison);
...
//		}
			...
			Output.get().logInfo("AutoManager: XPrison NukeTriggerEvent failed to load. " + "[%s] Caused by: [%s]",
					e.getMessage(), causedBy);
		}
	}
	...
	public void handleXPrisonNukeTriggerEvent(NukeTriggerEvent e, BlockBreakPriority bbPriority) {
		...
		MinesEventResults eventResults = ignoreMinesBlockBreakEvent(e, e.getPlayer(), e.getOriginBlock(), bbPriority,
				true);
		...
		debugInfo.append(String.format(
				"&6### ** handleXPrisonNukeTriggerEvent ** ###&3 "
		...
		// NOTE that check for auto manager has happened prior to accessing this
		// function.

		// Process all priorities if the event has not been canceled, and
		// process the MONITOR priority even if the event was canceled:
		if (!bbPriority.isMonitor() && !e.isCancelled() || bbPriority.isMonitor() && e.getBlocksAffected().size() > 0) {
