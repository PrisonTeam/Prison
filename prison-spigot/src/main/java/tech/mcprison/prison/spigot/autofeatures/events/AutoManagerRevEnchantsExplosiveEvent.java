...
//		boolean isEventEnabled = eP != null && !"DISABLED".equalsIgnoreCase( eP );
	...
	private void createListener( BlockBreakPriority bbPriority ) {
		...
		AutoManagerRevEnchantsExplosiveEventListener autoManagerListener = 
...
//    	super.unregisterListeners();
	...
	public void dumpEventListeners( StringBuilder sb ) {
		...
		try {
			...
			HandlerList handlers = ExplosiveEvent.getHandlerList();
			
//    		String eP = getMessage( AutoFeatures.blockBreakEventPriority );
    		BlockBreakPriority bbPriority = BlockBreakPriority.fromString( eP );

    		dumpEventListenersCore( "ExplosiveEvent", handlers, bbPriority, sb );
...
//			}
	...
	public void handleRevEnchantsExplosiveEvent(ExplosiveEvent e, BlockBreakPriority bbPriority) {
		...
		debugInfo.append(String.format(
				"&6### ** handleRevEnchantsExplosiveEvent ** ###&3 "
		...
		if (!bbPriority.isMonitor() && !e.isCancelled() || bbPriority.isMonitor() && e.getBlocks().size() > 0) {
