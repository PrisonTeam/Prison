...
import me.faizaand.prison.internal.inventory.InventoryHolder;
...
public class SpigotInventory implements Inventory {
    ...
    @Override public List<GamePlayer> getViewers() {
    ...
    @Override public boolean contains(GameItemStack itemStack) {
    ...
    @Override public Iterator<GameItemStack> getIterator() {
        ArrayList<GameItemStack> prisonStacks = new ArrayList<>();
    ...
    @Override public GameItemStack[] getItems() {
        ArrayList<GameItemStack> prisonStacks = new ArrayList<>();
        Arrays.asList(wrapper.getContents())
            .forEach(x -> prisonStacks.add(SpigotUtil.bukkitItemStackToPrison(x)));
        return prisonStacks.toArray(new GameItemStack[]{});
    }

    @Override public void setItems(List<GameItemStack> items) {
        List<org.bukkit.inventory.ItemStack> stacks = new ArrayList<>();
        items.forEach(x -> stacks.add(SpigotUtil.prisonItemStackToBukkit(x)));
    }

    @Override public HashMap<Integer, GameItemStack> getItems(BlockType type) {
        HashMap<Integer, GameItemStack> result = new HashMap<>();
        List<GameItemStack> items = Arrays.asList(getItems());
        items.removeIf(x -> x.getMaterial() != type);
        items.forEach(y -> result.put(items.indexOf(y), y));
        return result;
    }

    @Override public HashMap<Integer, GameItemStack> getItems(GameItemStack stack) {
        HashMap<Integer, GameItemStack> result = new HashMap<>();
        List<GameItemStack> items = Arrays.asList(getItems());
    ...
    @Override public void clear(GameItemStack stack) {
        wrapper.remove(SpigotUtil.prisonItemStackToBukkit(stack));
    }

    @Override public int first(GameItemStack stack) {
    ...
    @Override public Iterator<GameItemStack> iterator() {
        return getIterator();
    }
